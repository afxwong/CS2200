...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        for (size_t i = 1; i <= num_entries; i++) {
            pfn_t possible_victim = (i + last_evicted) % num_entries;
            if (!frame_table[possible_victim].protected) {
                last_evicted = possible_victim;
        ...
        for (size_t i = 1; i <= num_entries * 2; i++) {
            pfn_t possible_victim = (i + last_evicted) % num_entries;
            if (!frame_table[possible_victim].protected && !frame_table[possible_victim].referenced) {
                last_evicted = possible_victim;
                return possible_victim;
            } else if (frame_table[possible_victim].referenced) {
                frame_table[possible_victim].referenced = 0;
            }
        }
    }
